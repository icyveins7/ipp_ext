...
int main()
{
    ipps::matrix<Ipp64f> x(1, 10, 1.0);
    ipps::matrix<Ipp64f> y(1, 10, 2.0);

    printf("------Copy elision by compiler if you construct along with the math\n");
    ipps::matrix<Ipp64f> z = x + y;

    printf("------Copy constructor\n");
    std::vector<ipps::matrix<Ipp64f>> v;
    v.push_back(x);
    ...
    ipps::matrix<Ipp64f> z2(2,20); // put some existing memory in to make sure its released safely
    ...
    ipps::matrix<Ipp64f> z3(std::move(x));
